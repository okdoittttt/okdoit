...
async def upload_file(page: Page, action: dict) -> None:
    ...
    Raises:
        ValueError: path가 비어있거나 실제로 존재하지 않는 파일인 경우
    ...
    if not paths:
        raise ValueError("upload_file: path가 비어있습니다")

    for p in paths:
        if not os.path.exists(p):
            raise ValueError(f"upload_file: 파일이 존재하지 않습니다: '{p}'")
