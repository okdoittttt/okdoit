...
def isolated_data_dir(
    ...
    """``OKDOIT_DATA_DIR`` 를 tmp_path 로 격리하고 관련 ``lru_cache`` 들을 비운다.

    DB 부트스트랩이나 ``settings.db_path`` / ``settings.screenshot_dir`` 를 거치는
    테스트에서 사용. ``get_settings`` / ``get_session_store`` 둘 다 ``lru_cache``
    라 다른 테스트가 먼저 호출했으면 굳어버린 인스턴스를 쓸 수 있어 명시적으로
    cache_clear 한다.
